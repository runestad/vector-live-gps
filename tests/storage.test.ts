...
const demo: Scenario = { id: "demo", name: "Demo", builtIn: true, position: { lat: 59.9, lng: 10.7 }, route: [], speed: 42, speedMode: "adaptive", adaptiveSpeedPreset: "Normal", loop: false, status: "Active", battery: 84, signal: "Strong", trackerName: "V", deviceId: "1", vehicle: "Van", registration: "—", note: "", appearance: DEFAULT_APPEARANCE, zoom: 14 };
...
test("migrates V1 without losing tracker icons and survives serialization", () => {
  ...
  assert.equal(data.current.speedMode, "adaptive");
  assert.equal(data.current.adaptiveSpeedPreset, "Normal");
